  ...
  <div className="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-2xl">
    ...
    <div className="flex justify-around text-center mb-6 p-3 bg-gray-900/50 rounded-lg">
      <div>
        ...
        <p className="font-bold text-lg text-white">{plan.sets}</p>
      </div>
      <div>
        <p className="text-gray-400 text-xs sm:text-sm">Reps</p>
        <p className="font-bold text-lg text-white">{plan.reps}</p>
      </div>
      <div>
        <p className="text-gray-400 text-xs sm:text-sm">Descanso</p>
        <p className="font-bold text-lg text-white">{plan.rest}</p>
      </div>
    </div>
      ...
      {plan.schedule.map((daily) => (
        <div key={daily.day} className="bg-gray-700 p-4 rounded-lg">
          <h3 className="font-bold text-white">
            {daily.day}: <span className="text-emerald-400">{daily.group}</span>
          </h3>
...
const RunningTable: React.FC<{ phase: RunningPhase }> = ({ phase }) => (
  <div className="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-2xl mt-8">
    ...
    <div className="bg-gray-700 p-4 rounded-lg text-gray-300 space-y-2">
      <p>
        <strong className="text-white">Aquecimento:</strong>{" "}
        {phase.details.aquecimento}
      </p>
      <p>
        <strong className="text-white">Treino Principal:</strong>{" "}
        {phase.details.treino}
      </p>
      <p>
        <strong className="text-white">Desaquecimento:</strong>{" "}
        {phase.details.desaquecimento}
      </p>
    </div>
  </div>
);
...
function App() {
  ...
  const currentWorkoutPlan = workoutPlans.find(
    (p) => p.month === selectedMonth
  )!;
  const currentRunningPhase = runningPlan.find(
    (p) => p.id === selectedRunningPhase
  )!;

  const workoutOptions = workoutPlans.map((p) => ({
    id: p.month,
    label: `Mês ${p.month}`,
  }));
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-8">
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight">
            Meu Plano de Treino Personalizado
          </h1>
        ...
        <main>
          ...
          <section id="running-plan" className="mt-12">
            ...
            <PlanSelector
              ...
              selected={selectedRunningPhase}
              onSelect={setSelectedRunningPhase}
            />
        ...
        <footer className="text-center mt-12 text-gray-500 text-sm">
          <p>
            Lembre-se de ouvir seu corpo e treinar com
            segurança.
          </p>
        </footer>
      </div>
