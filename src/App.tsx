...
import { type MonthlyPlan, type RunningPhase, type Exercise } from './types';
import Modal from './components/Modal'; // Certifique-se de que o Modal está em 'src/components/Modal.tsx'
...
// Componente para exibir a tabela do plano de musculação (com lógica do Modal)
const WorkoutTable: React.FC<{ plan: MonthlyPlan }> = ({ plan }) => {
  const [modalData, setModalData] = useState<{ isOpen: boolean; exercise: Exercise | null }>({
    isOpen: false,
    exercise: null,
  });
  ...
  const openModal = (exercise: Exercise) => {
    if (exercise.gifUrl) {
      setModalData({ isOpen: true, exercise });
    }
  };

  const closeModal = () => {
    setModalData({ isOpen: false, exercise: null });
  };
    ...
    <>
      {modalData.exercise && (
        <Modal
          ...
          onClose={closeModal}
          exerciseName={modalData.exercise.name}
          gifUrl={modalData.exercise.gifUrl!}
        />
      )}
              ...
              <ul className="mt-2 list-disc list-inside text-gray-300 space-y-1">
                {daily.exercises.map((ex) => (
                  <li
                    ...
                    className={ex.gifUrl ? 'cursor-pointer hover:text-white' : ''}
                  >
                    {ex.name}
                    {ex.gifUrl && <span className="text-xs text-cyan-400 ml-2">(ver vídeo)</span>}
...
function App() {
  ...
  const workoutOptions = workoutPlans.map((p) => ({ id: p.month, label: p.title }));
